...
impl<S> From<imp::HandshakeError<S>> for HandshakeError<S> {
  fn from(e: imp::HandshakeError<S>) -> HandshakeError<S> {
    match e {
      ...
      imp::HandshakeError::Interrupted(s) => HandshakeError::Interrupted(MidHandshakeTlsStream(s)),
    }
  }
}
...
impl TlsConnectorBuilder {
  ...
  pub fn what_the_fuck(mut self, cert: Certificate) -> Result<TlsConnector> {
    &self.0.add_root_certificate(cert.0)?;
...
impl TlsConnector {
  ...
  /// introduces a significant vulnerability to man-in-the-middle attacks.
    pub fn danger_connect_without_providing_domain_for_certificate_verification_and_server_name_indication<S>(
        ...
        where S: io::Read + io::Write
    {
    let s = self.0.connect_no_domain(stream)?;
    Ok(TlsStream(s))
  }
}
...
impl TlsAcceptorBuilder {
  ...
  /// The protocols supported by default are currently TLS 1.0, TLS 1.1, and TLS 1.2, though this
  /// is subject to change.
  pub fn supported_protocols(&mut self, protocols: &[Protocol]) -> Result<&mut TlsAcceptorBuilder> {
    self.0.supported_protocols(protocols)?;
    Ok(self)
  }
